...
require(ggfortify)  # autoplot para PCA
require(purrr)      # map_dbl, map, etc.
require(factoextra) # funciones adicionales para K-means y PCA
```
...
```{webr-r}
## filtrar variables
features <- select(.data=spotify , 
                   danceability, energy, valence, acousticness, liveness, speechiness, tempo)
...
En este bloque se realiza el entrenamiento del modelo K-means fijando el número de clusters en k = 4. Primero, se establece una semilla aleatoria (set.seed(2024)) para asegurar que los resultados sean reproducibles, ya que K-means depende de inicializaciones aleatorias de los centroides.
...
```{webr-r}
## plot
ggplot(spotify_c, aes(x = popularity, y = streams, color = cluster)) +
...
Preguntas de interpretación:
	...
	•	¿Qué cluster concentra las canciones más escuchadas?

	2.	Duración: A partir de la mediana de duration_ms por clúster:
	•	¿Qué cluster tiende a tener canciones más largas?
	•	¿Cuál parece tener canciones más cortas?
